...
* 类型(type)：一个索引可以有1个或多个类型，是文档的逻辑分类，类似于mysql的表；7.0版本删除类型，一个索引只能一个类型
...
* 文档(doc)：一条记录，是es中数据存储和检索的基本单位；相当于mysql的一行记录；以json格式存储，由元数据(metadata)和数据(_source)组成；元数据包括：
  ...
  * _id：文档id
  * _version：文档版本
* 映射(mapping)：将文档转变为索引的过程，类似于创建表结构，对数据进行结构化存储
...
## [节点(node)](https://www.elastic.co/guide/en/elasticsearch/reference/6.5/modules-node.html)
